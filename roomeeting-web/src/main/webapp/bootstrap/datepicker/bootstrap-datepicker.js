...
!function( jQuery ) {
	...
	var Datepicker = function(element, options) {
		...
		jQuery(document).on('mousedown', function (e) {
			...
			if (jQuery(e.target).closest('.datepicker').length === 0) {
				that.hide();
			}
		});
...
//			});
		...
		click: function(e) {
			e.stopPropagation();
			e.preventDefault();
			var target = jQuery(e.target).closest('span, td, th');
			if (target.length == 1) {
	...
	jQuery.fn.datepicker = function ( option ) {
		...
		return this.each(function () {
			var jQuerythis = jQuery(this),
				data = jQuerythis.data('datepicker'),
				options = typeof option == 'object' && option;
			if (!data) {
				jQuerythis.data('datepicker', (data = new Datepicker(this, jQuery.extend({}, jQuery.fn.datepicker.defaults,options))));
			}
	...
	var dates = jQuery.fn.datepicker.dates = {
		en: {
		...
		parseDate: function(date, format, language) {
			...
			if (parts.length != fparts.length) {
				fparts = jQuery(fparts).filter(function(i,p){
					return jQuery.inArray(p, setters_order) !== -1;
				}).toArray();
			}
			// Process remainder
			if (parts.length == fparts.length) {
	...
	jQuery.fn.datepicker.DPGlobal = DPGlobal;
