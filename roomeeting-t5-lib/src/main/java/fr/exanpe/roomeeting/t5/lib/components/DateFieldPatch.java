package fr.exanpe.roomeeting.t5.lib.components;
...
/**
 * A component used to collect a provided date from the user using a client-side JavaScript
 * calendar. Non-JavaScript
 * clients can simply type into a text field.
 * <p/>
 * One wierd aspect here is that, because client-side JavaScript formatting and parsing is so
 * limited, we (currently) use Ajax to send the user's input to the server for parsing (before
 * raising the popup) and formatting (after closing the popup). Weird and inefficient, but easier
 * than writing client-side JavaScript for that purpose.
 * <p/>
...
@Import(library =
{ "js/dateFieldPatch.js", "js/datePickerPatch.js" }, stylesheet = "css/datePickerPatch.css")
@Events(EventConstants.VALIDATE)
public class DateFieldPatch extends AbstractField
{
    /**
    ...
    @Parameter(defaultPrefix = BindingConstants.ASSET, value = "images/datefield.gif")
    private Asset icon;
    ...
    void beginRender(MarkupWriter writer)
    {
        ...
        // On détermine le 1er jour du mois si la localisation ne fonctionne pas
        int firstDayPatch = 0;
        spec.put("firstDayPatch", firstDayPatch);
