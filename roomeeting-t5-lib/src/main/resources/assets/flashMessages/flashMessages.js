...
Growler = (function(){
	...
	return Class.create({
		initialize: function(options){
			...
			this.growler.setStyle({ position: ((IE)?"absolute":"fixed"), padding: "10px", "width": opt.width, "z-index": "500" });
			switch(opt.location){
				case "br":
				...
				case "bl":
					...
					break;
				case "tc":
					this.growler.setStyle({top: 0, left: ((IE)?"35%":"25%"), width: "50%"});
					break;
				case "bc":
					this.growler.setStyle({bottom: 0, left: ((IE)?"35%":"25%"), width: "50%"});
					break;
				default:
					this.growler.setStyle({top: 0, right: 0});
					break;
		...
		,flashNotice: function(msg, options){
			return specialNotice(this.growler, msg, options, "", "#2a2863", "#fff");
		}
...
FlashMessages = {
	display : function(spec) {
		var g = new Growler({location:"tc"});
		for ( var i = 0; i < spec.informations.length; ++i) {
			g.flashNotice(spec.informations[i], {life: 3.5});
		}
	}
};
